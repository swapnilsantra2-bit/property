...
// ============================================================
// Property Registry — Contract Methods
// ============================================================

/**
 * Register a new property. Caller becomes the owner.
 * Permissionless — anyone can register without approval.
 * Calls: register(caller: Address, property_id: String, location: String, description: String)
 */
 ...
 * Only the current owner can transfer (requires their auth).
 * Calls: transfer(caller: Address, property_id: String, new_owner: Address)
 */
export async function transferProperty(
  ...
  return callContract(
    "transfer",
    [toScValString(propertyId), toScValAddress(newOwner)],
    caller,
    true
  );
}
 ...
 * Calls: get_property(property_id: String) -> Property { location: String, description: String, owner: Address }
 * Returns: { location: string, description: string, owner: string } or null
 */
export async function getProperty(propertyId: string, caller?: string) {
  return readContract("get_property", [toScValString(propertyId)], caller);
}

/**
 * List all registered property IDs (read-only).
 * Calls: get_all_properties() -> Vec<String>
 * Returns: string[] or null
 */
export async function getAllProperties(caller?: string) {
  return readContract("get_all_properties", [], caller);
}
